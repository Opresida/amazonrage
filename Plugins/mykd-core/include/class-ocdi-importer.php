...
class GENIX_OCDI_Demo_Importer {
    ...
    public function import_files_config() {
		...
		$home_prevs = array(
			'genix_demo_home1' => array(
				...
				'preview_link' => 'https://themedox.com/mykd/',
			),
		);
    ...
    public function ocdi_after_import_setup( $selected_file ) {
        ...
        update_option( 'mykd_ocdi_importer_flash', true );
    }
    ...
    public function genix_ocdi_rewrite_flush() {
        ...
        if ( get_option( 'mykd_ocdi_importer_flash' ) == true ) {
            flush_rewrite_rules();
            delete_option( 'mykd_ocdi_importer_flash' );
        }
    }
}
